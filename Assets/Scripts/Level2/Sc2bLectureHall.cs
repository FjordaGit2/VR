...
/// <summary>
/// Inverted go/no-go vs <see cref="Sc2aLectureHall"/>: digit 3 = withhold response (no-go); digits 1,2,4,5,6,7,8,9 = press within the response window (go).
/// Same pseudorandom sequence rules and session CSV layout as Sc2a; behavioural folder is <c>Sc2bLectureHall</c>.
/// In <c>task_trials.csv</c>, column <c>target</c> is 1 when a keypress is the correct hit (Sc2a: shown digit 3; Sc2b: shown digit ≠ <see cref="noGoDigit"/>).
/// Outcomes: Hit = go + response; Miss = go + no response; False alarm = no-go + response; Correct rejection = no-go + no response.
/// </summary>
public class Sc2bLectureHall : LevelScript
{
    ...
    [Space]
    [Header("Trial counts (810 = 162× no-go + 8×81 go digits)")]
    [Min(1)] public int totalTrials = 810;
    [Min(1)] public int noGoDigit = 3;
    [Min(0)] public int noGoTrialCount = 162;
    [Tooltip("Each go digit (1–9 except noGoDigit) appears this many times. Required: noGoTrialCount + trialsPerGoDigit × 8 = totalTrials.")]
    [Min(0)] public int trialsPerGoDigit = 81;
    ...
    [Tooltip("Must divide totalTrials and noGoTrialCount for strict per-stratum balancing of no-go trials.")]
    ...
    public new void StartTask()
    {
        ...
        StartSessionRecordingIfEnabled("Sc2b");
    ...
    static void BeginLectureIfPresent()
    {
        ...
        if (lecture == null)
        {
            Debug.LogWarning("Sc2bLectureHall: no AnimAndImage found; lecture will not start.");
            return;
        }
    ...
    {
        const int goDigitKinds = 8;
        int expectedTotal = noGoTrialCount + trialsPerGoDigit * goDigitKinds;
        if (expectedTotal != totalTrials)
        {
            Debug.LogWarning(
                $"Sc2bLectureHall: totalTrials ({totalTrials}) != noGoTrialCount ({noGoTrialCount}) + trialsPerGoDigit×{goDigitKinds} ({trialsPerGoDigit * goDigitKinds}). " +
                $"Expected sum {expectedTotal}. Sequence build may be wrong; fix Inspector counts.");
        }

        if (totalTrials <= 0 || noGoTrialCount < 0 || trialsPerGoDigit < 0 || noGoDigit < 1 || noGoDigit > 9)
        {
            Debug.LogError("Sc2bLectureHall: invalid trial counts or noGoDigit.");
            _trialDigits = new List<int>();
            return;
        }

        if (totalTrials % targetStrataCount != 0 || noGoTrialCount % targetStrataCount != 0)
        {
            Debug.LogWarning(
                $"Sc2bLectureHall: targetStrataCount ({targetStrataCount}) should divide totalTrials ({totalTrials}) and noGoTrialCount ({noGoTrialCount}) for even no-go spacing. Stratum check relaxed.");
        }
        ...
        if (!ok)
            Debug.LogError("Sc2bLectureHall: could not satisfy sequence constraints; using last attempt. Consider more shuffle attempts or looser strata.");
    }
    ...
    List<int> BuildConstrainedSequence(int seed, out bool success)
    {
        ...
        for (int i = 0; i < noGoTrialCount; i++)
            multiset.Add(noGoDigit);
        for (int d = 1; d <= 9; d++)
        {
            if (d == noGoDigit)
                continue;
    ...
    bool PassesConstraints(List<int> seq)
    {
        ...
        if (totalTrials % targetStrataCount == 0 && noGoTrialCount % targetStrataCount == 0)
        {
            ...
            for (int s = 0; s < targetStrataCount; s++)
            {
                ...
                {
                    if (seq[start + i] == noGoDigit)
                        c++;
                }
                if (Mathf.Abs(c - expectedNoGo) > targetStrataCountTolerance)
                    return false;
            }
        }
    ...
    IEnumerator RunTaskCoroutine()
    {
        ...
        if (stimSec + postStimulusBlankMs * 0.001f > windowSec + 0.0001f)
            Debug.LogWarning("Sc2bLectureHall: stimulusDurationMs + postStimulusBlankMs exceeds responseWindowMs; window ends before blank ISI completes.");

        if (_trialDigits == null || _trialDigits.Count != totalTrials)
        {
            Debug.LogError("Sc2bLectureHall: invalid sequence; aborting task.");
            _csvSessionLogging = false;
            CloseSessionCsvWriters();
            yield break;
        }
        ...
        for (int trialIndex = 0; trialIndex < totalTrials; trialIndex++)
        {
            ...
            int goTrialFlag = digit != noGoDigit ? 1 : 0;
    ...
    void WriteSc2bSummaryCsv()
    {
        ...
        {
            Debug.LogWarning($"Sc2bLectureHall: failed to save sc2b_summary.csv ({e.Message})");
        }
    }
